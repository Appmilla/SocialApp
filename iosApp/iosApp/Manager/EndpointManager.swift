...
struct EnvironmentSettingModel: Codable {
    ...
    static var defaultSettings: EnvironmentSettingModel {
        ...
        let configSetting = EnvironmentSettingModel(configMap: configMap, selectedEnv: .eu)
        return configSetting
    }
    
    static func defaultConfig(for environment: EnvironmentType) -> EndpointConfigModel {
        return EndpointConfigModel(apiKey: "YOUR_API_KEY", httpEndpoint: AmityRegion.EU.httpUrl, socketEndpoint: AmityRegion.EU.rpcUrl, mqttEndpoint: AmityRegion.EU.mqttHost)
    }
}
